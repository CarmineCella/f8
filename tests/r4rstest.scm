;; R4RS compatibility adapted fro Vuza
;;
...
(define disjoint-type-functions
  (list boolean? null? number? pair? procedure? string? symbol? vector?)) ;; CEC

(define type-examples
  (list
   #t #f '() 9739 '(test) record-error "test" "" 'test '(a b c) )) ;; CEC
   																   ;; reduced
(define i 1)
...
(for-each (lambda (x y)
            ...
            (for-each (lambda (f)
                        ...
                        (cond ((and (= i j))
                              ...
                              ((f y) (test #f f y))))
                      disjoint-type-functions))
          (list #t '() 9739 '(test) record-error "test" 'car '(a b c))
		  (list #f '() -3252 '(t . t) car "" 'nil ))
(SECTION 4 1 2)
...
;       'quasiquote `(a `(b ,(+ 1 2) ,(foo ,(+ 1 3) d) e) f)) ;; CEC out
; (test '(a `(b ,x ,'y d) e) 'quasiquote
;         (let ((name1 'x) (name2 'y)) `(a `(b ,,name1 ,',name2 d) e)))  ;; CEC
...
(test 4 foo 4)
(test 34 'define x)
;(test 99 'internal-define (letrec ((foo (lambda (arg)                   ;; CEC
...
;(test-eq?-eqv?-agreement '#(a) '#(a)) ;; CEC out
;(test-eq?-eqv?-agreement '#(a) '#(b)) ;;
...
;(define (string-standard-case s)                                     ;; CEC out
;  (set! s (str-copy s))
;  (do ((i 0 (+ 1 i))
;       (sl (string-length s)))
;      ((>= i sl) s)
...
(test #t negative? -4)
(test #f negative? 0)
;(test #t odd? 3)					;; CEC out provisional
;(test #f odd? 2)
;(test #f odd? -4)
;(test #t odd? -1)
;(test #f even? 3)
...
(test 4 gcd 0 4)
(test 4 gcd -4 0)						;; CEC out provisional
(test 4 gcd 32 -36)
(test 0 gcd)
(test 288 lcm 32 -36)
(test 1 lcm)
...
(define (test-inexact)
  ...
  (test #t call-with-output-file
      "tmp3"
      (lambda (test-file)
        (write-char ";" test-file) ;; CEC modified
        (display ";" test-file)
        (display ";" test-file)
...
;(test #\a char-downcase #\a)
(SECTION 6 7)
(test #t string? "The word recursion has many meanings.")    ;; CEC modified
(test #t string? "")
(define f (make-string 3 "*"))								 ;; CEC modified
...
;;(test "" substring "ab" 0 0) ;; CEC out (for no reason??)
...
;(test #t call-with-current-continuation procedure?) ;; CEC out
...
(test '(0 1 4 9 16) 'for-each
        (let ((v (make-vector 5)))
                (for-each (lambda (i) (vector-set! v i (* i i)))
                        '(0 1 2 3 4))
                v))
;(test -3 call-with-current-continuation 							;; CEC out
;                (lambda (exit)										;;
...
;(define (next-leaf-generator obj eot)  ;; CEC out
;  (letrec ((return #f)
;           (cont (lambda (x)
;                   (recur obj)
...
(define (check-test-file name)
  ...
  ;(test #\; read-char test-file) ;; CEC out
...
(define write-test-obj
  '(#t #f a 9739 -3 ((test) "te" "st" test () b c))) ;; CEC modified
(define load-test-obj
  (list 'define 'foo (list 'quote write-test-obj)))
(test #t call-with-output-file
      "tmp1"
      (lambda (test-file) 
        ;(write-char #\; test-file) 						 ;; CEC out
        ;(display #\; test-file)							 ;; 
        (display ";" test-file)
        (write write-test-obj test-file)
        (newline test-file)
...
(define (test-sc4)
  ...
  (newline)
  (SECTION 6 7)
  (test '("P" " " "l") string->list "P l")
  (test '() string->list ""))
  ;(test "1\\\"" list->string '("1" "\\" \""")) ;; CEC out
  (test "" list->string '())
  (SECTION 6 8)
